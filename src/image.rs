...
pub fn find_optimal_image_layout(usage: vk::ImageUsageFlags) -> vk::ImageLayout {
    let sampled = usage.contains(vk::ImageUsageFlags::SAMPLED) || usage.contains(vk::ImageUsageFlags::INPUT_ATTACHMENT);
...
impl RenderingDevice {
    ...
    pub fn new_image_info(&self, mut info: vk::ImageCreateInfo, location: MemoryLocation) -> Image {
        unsafe {
            ...
            self.record(|encoder| {
                encoder.image_barrier(&res, vk::ImageLayout::UNDEFINED, res.optimal_layout);
            });
            res
        }
    }
